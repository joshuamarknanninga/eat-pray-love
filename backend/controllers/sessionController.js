...
exports.leaveSession = async (req, res) => {
    ...
    try {
        ...
        session.participants = session.participants.filter(participant => participant.toString() !== userId.toString());
        ...
        if (session.createdBy.toString() === userId.toString()) {
            if (session.participants.length > 0) {
                ...
                return res.json({ message: 'You left the session. Session has been deleted as there are no participants left.' });
            }
        }
